...
// 게임 관련 상태를 저장하는 Map들
const matchSockets = new Map<number, Map<number, WebSocket>>();
const matchStates = new Map<number, GameState>();
const gameIntervals = new Map<number, NodeJS.Timeout>();
const playerAuthenticated = new Map<number, Set<number>>();
const paddleDirections = new Map<number, Map<number, PaddleDirection>>();
const authTimeouts = new Map<number, Map<number, NodeJS.Timeout>>();
...
class GameService {
   ...
   * 플레이어 연결 등록
   */
  registerPlayerConnection(matchId: number, userId: number, socket: any): void {
    // 인증 타임아웃 설정
    this.setAuthTimeout(matchId, userId, socket);
    ...
    if (!matchStates.has(matchId)) {
      this.initGameState(matchId);
    }

    // 패들 방향 초기화
    if (!paddleDirections.has(matchId)) {
      paddleDirections.set(matchId, new Map());
    }
    paddleDirections.get(matchId)?.set(userId, 'stop');
  }

  /**
   * 인증 타임아웃 설정
   */
  setAuthTimeout(matchId: number, userId: number, socket: WebSocket): void {
    if (!authTimeouts.has(matchId)) {
      authTimeouts.set(matchId, new Map());
    }

    // 기존 타임아웃이 있으면 제거
    const userTimeouts = authTimeouts.get(matchId)!;
    ...
    const timeout = setTimeout(() => {
      const authenticated = playerAuthenticated.get(matchId);
      if (!authenticated || !authenticated.has(userId)) {
        socket.send(
          JSON.stringify({
            type: 'auth_error',
            data: '인증 시간이 초과되었습니다.',
  ...
  async authenticatePlayer(matchId: number, userId: number): Promise<{ isGameReady: boolean }> {
    // 인증 정보 저장
    if (!playerAuthenticated.has(matchId)) {
      playerAuthenticated.set(matchId, new Set());
    }
    ...
    // 모든 플레이어가 인증되었는지 확인
    const gameState = matchStates.get(matchId);
  ...
  updatePaddleDirection(matchId: number, userId: number, direction: PaddleDirection): void {
    const directions = paddleDirections.get(matchId);
    if (directions) {
  ...
  cleanupMatch(matchId: number): void {
    // 게임 타이머 정리
    const interval = gameIntervals.get(matchId);
    if (interval) {
      clearInterval(interval);
      gameIntervals.delete(matchId);
    }

    // 타임아웃 정리
    if (authTimeouts.has(matchId)) {
      const timeouts = authTimeouts.get(matchId)!;
      timeouts.forEach((timeout) => clearTimeout(timeout));
      authTimeouts.delete(matchId);
    }

    // 데이터 정리
    matchStates.delete(matchId);
    playerAuthenticated.delete(matchId);
    paddleDirections.delete(matchId);
  ...
  async initGameState(matchId: number): Promise<void> {
    try {
      ...
      if (!match || match.matchUsers.length < 2) {
        throw new GlobalException(GlobalErrorCode.MATCH_ALREADY_COMPLETE);
      }
      ...
      const gameState: GameState = {
        player1: {
          ...
          score: 0,
          userId: match.matchUsers[0].userId,
        },
        player2: {
          y: CANVAS_HEIGHT / 2 - PADDLE_HEIGHT / 2,
          score: 0,
          userId: match.matchUsers[1].userId,
        },
        ball: {
          x: CANVAS_WIDTH / 2,
          y: CANVAS_HEIGHT / 2,
      ...
      matchStates.set(matchId, gameState);
      playerAuthenticated.set(matchId, new Set());
      paddleDirections.set(matchId, new Map());
    } catch (error) {
      console.error(`게임 상태 초기화 실패: ${error.message}`);
      throw error;
    }
  }
    ...
    try {
      ...
      const countdownInterval = setInterval(() => {
        ...
        if (countdown < 0) {
          ...
          // 게임 루프 시작
          const interval = setInterval(() => this.updateGameLoop(matchId), 1000 / 60); // 60fps
          gameIntervals.set(matchId, interval);
        }
      }, 1000);
    } catch (error) {
  ...
  updateGameLoop(matchId: number): void {
    ...
    // 패들 이동 업데이트
    this.updatePaddles(matchId, gameState);

    // 공 이동 및 충돌 처리
    this.updateBallPosition(gameState);

    // 득점 체크
    this.checkScoring(matchId, gameState);

    // 게임 종료 체크
    this.checkGameOver(matchId, gameState);
  ...
  async endGame(matchId: number): Promise<void> {
    ...
    try {
      ...
      // 플레이어 점수 기록
      await this.updateMatchResults(matchId, gameState);
  ...
  async updateMatchResults(matchId: number, gameState: GameState): Promise<void> {
    ...
    if (gameState.winner === gameState.player1.userId) {
      await this.updatePlayerStats(gameState.player1.userId, gameState.player2.userId);
    } else {
      await this.updatePlayerStats(gameState.player2.userId, gameState.player1.userId);
    }
  }
